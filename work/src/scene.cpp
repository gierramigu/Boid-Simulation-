...
Scene::Scene() {
	...
	//cgra::mesh_builder simple_boid_md = cgra::load_wavefront_data(CGRA_WORKDIR + string("res/models/teapot.obj"));
	//m_simple_boid_mesh = simple_boid_md.build();
...
void Scene::loadCore() {
	...
	m_boids.clear();
	for (int i = 0; i < 100; i++) {
		m_boids.push_back(Boid(linearRand(vec3(-bound()), vec3(bound())), sphericalRand(1.0)));
	}
...
void Scene::loadCompletion() {
	...
	other_boids.clear();
	for (int i = 0; i < 100; i++) {
		other_boids.push_back(Boid(linearRand(vec3(-bound()), vec3(bound())), sphericalRand(1.0)));
	}
...
void Scene::draw(const mat4 &proj, const mat4 &view) {
	...
	if (m_show_skymap) {
		static GLuint tex = 0;
		if (!tex) {
			tex = cgra::rgba_image(CGRA_WORKDIR + string("res/textures/sky.jpg")).uploadTexture(); //sky
			//tex = cgra::rgba_image(CGRA_WORKDIR + string("res/textures/space.jpg")).uploadTexture(); //space
		}
...
void Scene::renderGUI() {
	...
	ImGui::SliderFloat("Cohesion", &m_cohesion, 0, 50.0);
	ImGui::SliderFloat("Alignment", &m_alignment, 0, 50.0);
	ImGui::SliderFloat("Avoidance", &m_avoidance, 0, 50.0);
	ImGui::SliderFloat("Maximum Speed", &maxSpeed, 0, 100.0);
	ImGui::SliderFloat("Minimum Speed", &minSpeed, 0, 100.0);
	ImGui::SliderFloat("Maximum Acceleration", &maxAcceleration, 0, 100.0);
	ImGui::SliderFloat("Maximum Force", &maxForce, 0, 100.0);
	ImGui::SliderFloat("Neighbour Size", &neighbourSize, 0, 100);
